    ...
    useEffect(() => {
        ...
        if(props.FromData && props.ToData) {
            ...
            months.forEach((mon, index) => {
                axios.get(`https://api.currencybeacon.com/v1/${new Date().getFullYear() - 1}-${index + 1}-${new Date(new Date().getFullYear() - 1, index + 1, 0).getDate()}?base=${props.FromData?.label}&symbols=${props.ToData?.label}&api_key=${access_key}`)
                        .then((res) => {
